...
| **Loading data into tables** | `CREATE STAGE ON SCHEMA`, `Create file FORMATGRANT CREATE FILE FORMAT ON SCHEMA <SCHEMA> to ROLE <ROLE>` `SELECT`, and `INSERT` on the target table privileges | `SELECT` and `INSERT` permissions | The role assigned to the service account has to contain: `bigquery.jobs.create`, `bigquery.tables.getData`, and `bigquery.tables.updateData` | `SELECT` and `MODIFY` permissions |
| **Accessing client data** | `SELECT on (FUTURE) TABLE(S)` or `VIEW(S)` privilege(s) | `SELECT` permission | The role assigned to the service account has to contain: `bigquery.jobs.create`, `bigquery.readsessions.create`, and `bigquery.tables.getData` for tables or the `bigquery.dataViewer` role | `SELECT` permission |
| **Accessing metadata** | `SELECT on INFORMATION_SCHEMA SCHEMA` privilege | `SELECT` permission | `bigquery.metadataViewer` role |  `SELECT on INFORMATION_SCHEMA SCHEMA` permission |
